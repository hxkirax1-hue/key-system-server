...
app.get('/api/keys', async (req, res) => {
  try {
    ...
    const keysList = Object.keys(data).map(k => ({ key: k, ...data[k] }));
    res.json(keysList);
  } catch (err) {
...
app.post('/api/verify', async (req, res) => {
  ...
  try {
    ...
    if (!snapshot.exists()) return res.json({ status: 'error', message: 'invalid' });
    ...
    if (device_id) {
      if (devices.includes(device_id)) return res.json({ status: 'success', message: 'valid' });
      if (devices.length >= keyData.maxDevices) return res.json({ status: 'error', message: 'limit' });
